...
import { Trash2, Star, MessageSquare, Search, Reply, Pencil, RefreshCw } from "lucide-react";
...
export default function AdminTestimonials() {
  ...
  const load = async () => {
    try {
      ...
      const sorted = (data || []).sort((a, b) => {
        ...
        return dateB - dateA || (b.rating || 5) - (a.rating || 5);
      });
  ...
  // Handle posting or updating administrative reply
  ...
  const filtered = items.filter(item => {
    const text = (item.comment || item.review || item.content || "").toLowerCase();
    const name = (item.name || item.user_name || "").toLowerCase();
    const role = (item.role || "").toLowerCase();
    const query = search.toLowerCase();

    return !search || name.includes(query) || text.includes(query) || role.includes(query);
  });
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="relative w-full sm:w-72">
          ...
          <Input 
            ...
            className="pl-9 bg-muted border-border text-card-foreground rounded-full" 
          />
        </div>
        <div className="flex items-center gap-3 text-muted-foreground text-sm font-medium">
          ...
          <span>Showing {filtered.length} of {items.length} reviews</span>
        </div>
      </div>
          ...
          <div key={item.id} className="bg-card border border-border rounded-2xl p-5 flex flex-col justify-between hover:shadow-md transition-shadow">
            <div>
              <div className="flex items-start justify-between gap-2 mb-2">
                <div>
                  <h3 className="text-card-foreground font-black text-base">{item.name || item.user_name || "Anonymous"}</h3>
              ...
              <p className="text-card-foreground/90 text-sm italic bg-muted/30 p-3 rounded-xl border border-border/40 mb-3">
                "{item.comment || item.review || item.content}"
              </p>
            ...
            <div className="flex items-center justify-between border-t border-border/60 pt-3 mt-2 flex-wrap gap-2">
              <div className="flex items-center gap-2">
                <button 
                  onClick={() => { setReplyingTo(item); setReplyText(item.admin_reply || ""); }} 
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-muted text-muted-foreground hover:text-card-foreground hover:bg-muted/80 text-xs font-bold transition-all"
                >
          ...
          {replyingTo && (
            <form onSubmit={handleReply} className="space-y-4 mt-2">
              <div className="bg-muted/40 border border-border rounded-xl p-3 text-sm">
                <p className="font-bold text-card-foreground mb-1">{replyingTo.name || replyingTo.user_name || "Customer"}</p>
                <p className="text-muted-foreground italic">"{replyingTo.comment || replyingTo.review || replyingTo.content}"</p>
              </div>
              <div>
                <Label className="text-muted-foreground text-xs uppercase tracking-wide">Your Reply</Label>
                <Textarea 
                  value={replyText} 
          ...
          {editingItem && (
            <form onSubmit={handleSaveEdit} className="space-y-4 mt-2">
              <div>
                ...
                <Input value={editForm.name || editForm.user_name || ""} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))} className="bg-muted border-border text-card-foreground mt-1" required />
              </div>
              <div>
                <Label className="text-muted-foreground text-xs uppercase tracking-wide">Customer Role / Subtitle</Label>
                <Input value={editForm.role || ""} onChange={e => setEditForm(f => ({ ...f, role: e.target.value }))} className="bg-muted border-border text-card-foreground mt-1" placeholder="e.g. Verified Buyer" />
              </div>
              <div>
              ...
              <div>
                ...
                <Textarea value={editForm.comment || editForm.review || editForm.content || ""} onChange={e => setEditForm(f => ({ ...f, comment: e.target.value, review: e.target.value }))} className="bg-muted border-border text-card-foreground mt-1 min-h-[90px]" required />
              </div>
              <div>
                <Label className="text-muted-foreground text-xs uppercase tracking-wide">Display Settings</Label>
                <select value={String(editForm.is_featured ?? false)} onChange={e => setEditForm(f => ({ ...f, is_featured: e.target.value }))} className="w-full bg-muted border border-border text-card-foreground text-sm rounded-xl px-3 py-2 mt-1 focus:outline-none focus:ring-2 focus:ring-accent/40">
                  ...
                  <option value="true">Featured Showpiece</option>
                </select>
              </div>
              <div className="flex gap-3 pt-2">
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl p-8 max-w-sm w-full mx-4 text-center shadow-2xl">
            ...
            <p className="text-muted-foreground mb-6 text-sm">This review will be removed immediately.</p>
